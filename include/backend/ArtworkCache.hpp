...
struct ArtworkEntry {
    ...
    ArtworkEntry(std::vector<uint8_t> d, std::string mime, size_t refs = 0)
        : data(std::move(d)), mime_type(std::move(mime)), ref_count(refs) {}
};
...
public:
    ...
    void store(const std::string& hash, std::vector<uint8_t> data, const std::string& mime_type);
